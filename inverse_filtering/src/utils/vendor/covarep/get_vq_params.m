...
function [NAQ,QOQ,H1H2,HRF,PSP,ST] = get_vq_params(gf,gfd,fs,GCI)
...
min_harm_num=5;
HRF_freq_max=min(5000,fs/2); % Maximum frequency used for harmonic measurement
...
for n=1:length(GCI)
    ...
    if isinf(F0)==0 && T0~=0 && F0 > F0min && F0<F0max 
        ...
        if length(h_idx) >= min_harm_num
            ...
            % spectral tilt
            % linear regression is done on log10 scale to obtain the tilt on dB/decade
            %p=polyfit(1+log10((1:length(f0_idx))),h_amp(f0_idx),1); %IBERSPEECH22
            %x_log = 1+log10(h_idx(f0_idx)./F0); % MORE PRECISE
            x_log = 1+log10(1:length(f0_idx)); % equivalent to IBERSPEECH22 computation
            p=polyfit(x_log,h_amp(f0_idx),1);
            y=p(1)*x_log+p(2);
            ST(n)=p(1);
            if debug
                    figure(1);
                    subplot(211)
                    plot(gf_seg);axis tight;grid
                    title('Pulse waveform')

                    subplot(212);
                    plot((0:(round(length(f_spec)/2)-1)),f_spec(1:round(length(f_spec)/2)));
                    xline(h_idx(f0_idx),'r');
                    axis tight;grid
                    hold on
                    plot(F0*(10.^(x_log-1)),y,'g')
                    set(gca,'XScale','log')
                    xlim([100/1.8 8000/2]);
                    title(['Pulse spectrum [' num2str(p(1)) ' dB/decade]'])
            end
        end
...
while flag == 0
    ...
    if (NE < NE_min)
        if (N<length(X))
            ...
            disp('NE < NE_min dins de ger_vq_params.m')
            flag = 1;
        end
    else
        flag = 1;
