...
This directory containes all the necesary code to generate the extended version of the OPENGLOT repository I.
...
This folder containes the MATLAB code developed for running the optimization and the analysis of GIF algorithms.
...
In this folder you can find the csv files with the results used in our paper.
...
This projects allows to run the optimisation of parameters for the implemented GIF methods as well as the execution of said GIF methods given a set of parameters for the computation of a collection of error measures. This second action is called analysis in this project.

An example folder with the necessary files to execute the code is shared, called `experiment`.

## Execution

To execute the code the following is executed in MATLAB's Command Window:
...
The main `config.ini` file needs to be in a the main experiment folder (experiment_parent_directory). Subfolders will contain the rest of files. A first subfolder (GIF_config_files_dir) will contain the necesary GIF method configuration files.
In the of case executing an analysis another subfolder is needed (GIF_parameters_files_dir). This one contains the csv files (one per each GIF method) with the parameters per audio file in the corpus.
...
audio_files_directory_name
```

### config.ini

The experiment will run following this configuration file.
...
corpusDir = audio_files_directory_name		; e.g. wavs
...
resultsDir = results_directory_name			; e.g. optimizationResults or analysisResults
...
This is a template for the GIF configuration.
...
Note: the output csv files from the optimisation work as input for the analysis.

#### IAIF, IOP-IAIF and GFM-IAIF
`FileName	VTorder	GSorder	LipRad`

#### QCP and ST-QCP
`FileName	VTorder	GSorder	LipRad	DQ	PQ	RQ	STcompensation`
...
To be able to execute this code you will need access to these libraries:
